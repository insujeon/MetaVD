...
class VdGausPrior(BaseModel):
    def __init__(self, args, num_classes):
        ...
        self.model = CNNCifarVD(self.args.num_classes, gauss=True, prior=True)
        self.model = self.model.cuda()
        self.hpnet = VDModel(self.model, self.args.num_users, prior=True)
    ...
    def client_update(self, ldr_train, wt, w_local=None, iter=None, ldr_train_train=None, ldr_train_val=None):
            ...
            kl_loss = self.model.kl_div(wt[10], wt[11], wt[8])
    ...
    def server_aggregation(self, user_idxs, collection):
        ...
        vardist = torch.cat([collected_weights[i][10].clamp(-8,8).exp().unsqueeze(0) for i in range(self.args.frac_m)])
