...
class VdGausEmQ(BaseModel):
    def __init__(self, args, num_classes):
        ...
        args.exp += f"mom1:{args.momentum1}_decay1:{args.decay1}_"
        args.exp += f"beta:{args.beta}_cyc:{args.temp_cycle}_ratio:{args.temp_ratio}_"
        ...
        self.model = CNNCifarVD(self.args.num_classes, gauss=True)
        self.model = self.model.cuda()
        self.hpnet = VDModelEm(self.model, self.args.num_users)
        self.server_optimizer = torch.optim.SGD(
            [
                ...
                # {'params': self.hpnet.w_logits.parameters(), 'lr':self.args.server_lr, 'momentum':0.1, 'weight_decay':0}
