...
class PerFedAvgSnip(BaseModel):
    ...
    def client_update(self, ldr_train, wt, w_local=None):
        ...
        for i in range(numiter):
            ...
            # wt = [w - self.args.inner_lr * g * self.hpnet.mask if i==8 else w - self.args.inner_lr *g for i, (w,g) in enumerate(zip(wt, grads_))] # masking for pruned weight
            wt = [
                w - self.args.inner_lr * g * self.hpnet.mask[i] if i in self.hpnet.droplayer else w - self.args.inner_lr * g for i, (w, g) in enumerate(zip(wt, grads_))
            ...
            # wt = [w - self.args.client_lr*(g1 - self.args.inner_lr*g2)*self.hpnet.mask if i==8 else w - self.args.client_lr*(g1 - self.args.inner_lr*g2) for i, (w,g1,g2) in enumerate(zip(wt0, grads_1st, grads_2nd))] # masking for pruned weight
            wt = [
                w - self.args.client_lr * (g1 - self.args.inner_lr * g2) * self.hpnet.mask[i] if i in self.hpnet.droplayer else w - self.args.client_lr * (g1 - self.args.inner_lr * g2)
                for i, (w, g1, g2) in enumerate(zip(wt0, grads_1st, grads_2nd))
            ]  # masking for pruned weight
